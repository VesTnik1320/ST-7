package com.example;

import org.openqa.selenium.WebDriver;
import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;
import org.json.simple.JSONObject;
import org.json.simple.JSONArray;
import org.json.simple.parser.JSONParser;
import java.io.FileWriter;
import java.io.PrintWriter;
import java.nio.file.Files;
import java.nio.file.Paths;

public class Task3 {
    public static void run(WebDriver driver) {
        try {
            String url = "https://api.open-meteo.com/v1/forecast?latitude=56&longitude=44" +
                         "&hourly=temperature_2m,rain&current=cloud_cover" +
                         "&timezone=Europe%2FMoscow&forecast_days=1&wind_speed_unit=ms";
            driver.get(url);
            String jsonText = driver.findElement(By.tagName("body")).getText();
            JSONParser parser = new JSONParser();
            JSONObject root = (JSONObject) parser.parse(jsonText);
            JSONObject hourly = (JSONObject) root.get("hourly");
            JSONArray times = (JSONArray) hourly.get("time");
            JSONArray temperatures = (JSONArray) hourly.get("temperature_2m");
            JSONArray rains = (JSONArray) hourly.get("rain");

            Files.createDirectories(Paths.get("result"));
            PrintWriter writer = new PrintWriter(new FileWriter("result/forecast.txt"));

            System.out.println("\nПрогноз погоды на сутки (Нижний Новгород):");
            System.out.printf("%-4s %-20s %-12s %-10s%n", "№", "Дата/время", "Температура", "Осадки (мм)");
            writer.printf("%-4s %-20s %-12s %-10s%n", "№", "Дата/время", "Температура", "Осадки (мм)");

            for (int i = 0; i < times.size(); i++) {
                String time = (String) times.get(i);
                double temp = ((Number) temperatures.get(i)).doubleValue();
                double rain = ((Number) rains.get(i)).doubleValue();
                System.out.printf("%-4d %-20s %-12.1f %-10.2f%n", i+1, time, temp, rain);
                writer.printf("%-4d %-20s %-12.1f %-10.2f%n", i+1, time, temp, rain);
            }
            writer.close();
            System.out.println("\nТаблица сохранена в файл result/forecast.txt");
        } catch (Exception e) {
            System.out.println("Ошибка при получении погоды: " + e.toString());
        }
    }
}